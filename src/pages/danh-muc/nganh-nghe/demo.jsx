    ...
    return (
        <div className='w-full'>
            ...
            <div>
                <table className='border border-[#000] w-full'>
                    <thead>
                        <tr className='border border-[#000]'>
                            <th className='border border-[#000]'>STT</th>
                    ...
                    <tbody>
                        {data.length === 0 && (
                            <tr className='border border-[#000]'>
                                <td className='border border-[#000]' colSpan={4}>Không có dữ liệu</td>
                            </tr>
                        )}
                        {paginatedData.map((field, index) => (
                            <tr className='border border-[#000]'>
                                <td className='border border-[#000]'>{((currentPage - 1) * pageSize) + index + 1}</td>
                                <td className='border border-[#000]'><ButtonAction item={field} onDelete={handleDelete} clickUpdate={() => {
                                    setEditField(field);
                                    setShowModal(true)
                                }}/></td>
                                <td className='border border-[#000]'>{field.code}</td>
                                <td className='border border-[#000]'>{field.name}</td>
                            </tr>
                        ))}
                    </tbody>
                    <tfoot>
                            ...
                            <td colSpan={4}>
                                ...
                                {[...Array(totalPage)].map((_, index) => (
                                    <button
                                        ...
                                        className='border p-3 border-[#000]'>{index + 1}</button>
                                ))}
                                <button onClick={() => {
                ...
                {showModal && (
                    <DemoModal title={editField ? "Chỉnh sửa ngành nghề" : "Thêm mới ngành nghề"} onClose={handleClose}>
                        {editField ? (
                            <div>
                                <input value={editField.code} onChange={(e) => {
                                    setEditField(pre => ({ ...pre, code: e.target.value }))
                                }} autoFocus className='w-1/2 border p-2' placeholder='Mã ngành nghề' />
                                <input value={editField.name} onChange={(e) => {
                                    setEditField(pre => ({ ...pre, name: e.target.value }))
                                }} className='w-1/2 border p-2' placeholder='Tên ngành nghề' />
                                <button onClick={() => {
                                    handleUpdate()
                                }} className='w-1/2 border p-2'>{"Sửa"}</button>
                            </div>
                        ) : (
                                <div>
                                    ...
                                    <button onClick={() => {
                                        handleCreate()
                                    }} className='w-1/2 border p-2'>{"Thêm"}</button>
                                </div>
                        )}
                    </DemoModal>
                )}
        ...
        <div 
            ...
            {open && (
                <div style={{
                    ...
                    <div onClick={clickUpdate}>Sửa</div>
                    <div onClick={() => {
                        onDelete(item.id);
                        setOpen(false);
                    }}>Xoá</div>
